...
app = Flask(__name__)
CORS(app, resources={r"/process_input": {"origins": "http://localhost:5173"}})
@app.route('/')
def home(): 
...
def receiveChatData():
    msgs = str(request.form.get('messages'))  # Get the input from the front-end
    response = generate_res(msgs)  # Simulate some Python processing
    return msgs
